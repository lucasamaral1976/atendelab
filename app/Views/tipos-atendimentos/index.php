...
<div class="card border-0 shadow-sm mb-4 d-none" id="cardFormulario">
    <div class="card-body">
        ...
        <form id="formTipo">
            ...
            <div class="row g-3">
                <div class="col-md-6"><label class="form-label">Nome *</label><input class="form-control" name="nome" id="campoNomeTipo" required></div>
                <div class="col-md-3">
                    <label class="form-label">Status</label>
                    <select class="form-select" name="status" id="campoStatusTipo">
                        <option value="ativo">Ativo</option>
                        <option value="inativo">Inativo</option>
                    </select>
                </div>
                <div class="col-12"><label class="form-label">Descrição</label><textarea class="form-control" name="descricao" id="campoDescricaoTipo" rows="2"></textarea></div>
            </div>
...
async function editarTipo(id) {
    try {
        ...
        // BURLANDO O TOOBJECT: Lendo a resposta como lista e pegando a primeira posição
        const lista = AtendeLabApi.toList(resposta);
        const tipo = lista.length > 0 ? lista[0] : {};
        
        novoTipo();
        tituloFormulario.textContent = 'Editar tipo';
        
        // Mapeamento manual e seguro direto nos IDs dos inputs
        document.getElementById('tipoId').value = id;
        if(tipo.nome) document.getElementById('campoNomeTipo').value = tipo.nome;
        if(tipo.status) document.getElementById('campoStatusTipo').value = tipo.status;
        if(tipo.descricao) document.getElementById('campoDescricaoTipo').value = tipo.descricao;
...
formTipo.addEventListener('submit', async (event) => {
    ...
    // Se o ID oculto estiver preenchido, redireciona para atualizar em vez de duplicar
